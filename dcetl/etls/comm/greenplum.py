...
def update_table_info(conn, crt_tb_sql, schm_tb_nm, test_tb_nm, test_schm, trgt_tb, trgt_schm):
    ...
    if new_pk_df.shape[0] < 1:
        # 没有主键时，对greenplum分布键校验
        old_distributed = get_table_distributed_col(conn, trgt_tb, trgt_schm)
        new_distributed = get_table_distributed_col(conn, test_tb_nm, test_schm)
        if old_distributed != new_distributed:
            sql = """ ALTER TABLE {0} set {1} ;"""
            sql = sql.format(schm_tb_nm, new_distributed)
            conn.exec(sql, if_print_sql=False)
    logger.debug('删除测试表,完成表结构校验')
    conn.exec("DROP TABLE IF EXISTS {schm_nm}.{tb_nm};".format(schm_nm=test_schm, tb_nm=trgt_tb), if_print_sql=False)
...
def get_table_pk(conn, tb_nm, schm_nm=None):
    ...
    sql = """
     with pk as(       
            ...
            select pk.pk_name pk_nm,string_agg(a.attname,',') pk_cols from pk 
            left join pg_attribute a on pk.oid=a.attrelid and a.attnum=pk.col_id
            group by pk.pk_name
    """
...
def get_table_distributed_col(conn, tb_nm, schm_nm=None):
    """获取分布键"""
    if "." in tb_nm:
        tp = tb_nm.split(".")
        tb_nm = tp[1]
    ...
    with a as(
          SELECT
                ...
                aa.relname,
                bb.localoid,
                case when bb.distkey[0]  is null and bb.policytype='p'  then 'DISTRIBUTED RANDOMLY' 
                   when bb.policytype='r' then 'DISTRIBUTED REPLICATED' else 'DISTRIBUTED BY' end distributed_type,
                -- bb.policytype,
                case when bb.distkey[0] is null then '0' else
                regexp_split_to_table(
                    array_to_string(bb.distkey, ','),
                    ','
                )  end att,
                dd.nspname
            FROM
                pg_class aa --原数据信息 最重要的表！
            inner JOIN gp_distribution_policy bb ON bb.localoid = aa.oid and aa.relname = '{tb_nm}'
            inner JOIN pg_namespace dd ON dd.oid = aa.relnamespace AND dd.nspname='{schm_nm}'
            LEFT JOIN pg_inherits hh ON aa.oid = hh.inhrelid --继承表  
            where -- aa.relname='dim_org' and dd.nspname = 'edw' and
            hh.inhrelid IS NULL 
    )
    select  
        a.relname tb_nm,
        a.distributed_type distributed_by,
        string_agg(ccc.attname,',') cols
    from a
    LEFT JOIN pg_attribute ccc ON ccc.attrelid = a.oid
    AND ccc.attnum = a.att::int
    group by a.relname,a.distributed_type
    """
    sql = sql.format(tb_nm=tb_nm, schm_nm=schm_nm)
    df = conn.select(sql)
    if df.shape[0] > 0:
        distributed_by = df.loc[0, 'distributed_by']
        cols = df.loc[0, 'cols']
        if cols:
            distributed_by = distributed_by + " (" + cols + ")"
...
def generate_simple_ddl(trgt_tb_nm, tb_strct, meta):
    """获取简单的ddl语句，主要用于ods层同步数据，不会建主键等信息"""
    col_crt = []
    col_cmnts = []
